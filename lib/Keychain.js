...
class Keychain
{
    ...
    /**
     * Helper, tries to transform the source into a keychain and return it.
     * If source already was a keychain, the same instance is returned.
     * If the source is a NodeRSA instance, returns a new keychain with only that key in it.
     * @param {NodeRSA|Keychain} source
     * @return {Keychain|null}
     */
    static makeKeychainFrom(source)
    {
        if(source instanceof Keychain) {
            return source;
        } else if(source instanceof NodeRSA) {
            let keychain = new Keychain();
            keychain.addKeypair(source);
            return keychain;
        }
